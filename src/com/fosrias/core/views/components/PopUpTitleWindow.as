...
/**
 * The PopUpTitleWindow class is a <code>TitleWindow</code> subclass with 
 ...
 * @see com.fosrisa.core.views.PopUpCanvas
 ...
 */
public class PopUpTitleWindow extends TitleWindow implements IPopUpComponent
{
    //--------------------------------------------------------------------------
    //
    //  Constructor
    ...
    /**
     * @private
     * Storage for the resizeable property. 
     */
    private var _resizeable:Boolean = false;
    
    [Inspectable(category="General", defaultValue="false")]
    /**
     * Whether the component can be resized or not.
     */
    ...
    protected final function setPopUpsByArray( keyArray:Array,
        type:Object, propertyName:String ):void
    {
    ...
    {
    	addEventListener( MouseEvent.MOUSE_DOWN, resizeEventHandler, false, 0 , 
    	    true );
    }
    ...
    {
        removeEventListener( MouseEvent.MOUSE_DOWN, resizeEventHandler );
    }
    
    /**
     * @private
     */
    private function resizeEventHandler( event:MouseEvent ):void 
    {
        ...
        //Only resize if mouse clicked in lower righthand corner
        if ( upperLeftX <= panelRelX && panelRelX <= lowerLeftX )
        {
            if ( upperLeftY <= panelRelY && panelRelY <= lowerLeftY )
            {
                ...
                _origWidth = width;
                _origHeight = height;
                _xOffset = parent.mouseX;
                _yOffset = parent.mouseY;
    ...
    {   
        var newSize:int = _origWidth + parent.mouseX - _xOffset;
        
        if ( newSize > 250 && newSize > this.minWidth )
        {
                this.width = newSize; 
        }
        
        newSize = _origHeight + parent.mouseY - _yOffset;
        if ( newSize > titleBar.height && newSize > this.minHeight )
        {  
                this.height = newSize; 
        } 
    }
